        ...
        if has_resume_links:
            ...
            for row_idx, (blob_name, employee_id) in enumerate(zip(resume_paths, employee_ids), start=1):  # +1 for header row
                if blob_name and not pd.isna(blob_name) and employee_id and not pd.isna(employee_id):
                    # Link points at our own backend, which proxies the actual
                    # file from GCS — not a GCS signed URL, since generating
                    # one requires a signing-capable (service account)
                    # credential we don't currently have. This is the real
                    # .docx, not a rendered approximation: HR needs the exact
                    # file, not a preview that may render slightly differently.
